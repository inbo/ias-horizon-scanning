...
#Archaeophytes, species native in parts of the EU (not Canary Islands),
Irena_plants_exclude<-Irena_plants%>%
  filter(DECISION%in% c("arch","native","excludeHigherTaxa"))%>%
  pull(ID)

#Exclude these ID's from the longlist
longlist<-longlist%>%
  filter(!ID %in% Irena_plants_exclude)

grid_logs[1,2]<-length(unique(longlist$`Long name`))
```
...
#Only keep taxa that are mapped to species level: n = 7368
mapped_species<-mapped_taxa%>%
  filter(!is.na(speciesKey))

grid_logs[1,3]<-length(unique(mapped_species$speciesKey))
```
...
grid_logs[1,4]<-length(unique(mapped_species$speciesKey))
```
...
#Exclude these species from the final longlist that will be used in the download: 7039
mapped_species_final<-mapped_species%>%
  filter(!ID %in% Belgium_natives$ID)


grid_logs[1,6]<-length(unique(mapped_species_final$key))
```
...
```{r GBIF download}
#Extract taxonkeys as vector (length 7041, 6751)
...
#The download exceeds the available RAM in R (18 218 542 records), increase RAM to 13Gb
#edit_r_environ()
#Add =13Gb to the renviron file, save, and restart R session gbif_download_key<-"0025284-240906103802322"
...
#Records for 3535 (out of 7041) species are downloaded (the others don't have records in Europe, at least not for our selection criteria)
...
grid_logs[1,7]<-length(unique(species_records$speciesKey)) #3535; 3243
...
length(unique(species_records$speciesKey))
 #3487; 3166, 77 removed by coordinate cleaner
...
#Export the species for which no records are downloaded or for which all records were removed by coordinate cleaner or previous filtering steps (4504)
...
#Together with the 4504 species that were not included: 5077 species (here 6751, same is taxonkeys length for download, because still including those with more than two grid cells occupied)
...
grid_logs[1,8]<-length(unique(climate_matching$speciesKey))
