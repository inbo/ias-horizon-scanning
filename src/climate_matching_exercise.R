#----------------------------------------------------
#----------------------------------------------------
#-----Script for the climate matching excercise------
#----------------------------------------------------
#----------------------------------------------------

#----------------------------------------------
#----STEP 0: Load libraries and data layers----
#----------------------------------------------
...
mapview(region_shape)

#Load climate_matching file
climate_matching<-read_sheet("1TevRYtBiBUhiPI-lRbJPuhkNAAqBhz3g3kEyKZY42YU",sheet="Data")

#---------------------------------------------------------
#----STEP 1: Prepare blocks of taxonkeys for download ----
#---------------------------------------------------------
...
current<-future[["2026-2050-A1FI"]]%>%
  st_intersection(., region_shape)%>%
  mutate(GRIDCODE=as.integer(GRIDCODE))%>%
  left_join(.,KG_Rubel_Kotteks_Legend, by=c("GRIDCODE"))%>%
  dplyr::filter(Classification %in% c("Bsk", "BSh"))
mapview(current)
