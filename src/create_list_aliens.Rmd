...
griis_all<-synonym_fix%>%
  ...
  select(c(accepted_taxa_key, accepted_taxon_name, datasetKey,kingdom, phylum, order, family, scientificName,rank,taxonomicStatus, griis_country, class))%>%
...
griis_overview<-griis_taxa%>%
  ...
  group_by(accepted_taxa_key, accepted_taxon_name, kingdom, class, rank)%>%
...
griis_select<-hist(griis_overview$n_obs)
ggplot(griis_overview, aes())+
  geom_histogram()
```
