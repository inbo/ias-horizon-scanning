...
for (i in unique(maps_data$ID)){
  ...
  if(thematic_group %in% c("Marine")){
    ...
    plot_final<-plot1 /empty_plot +   plot_annotation( title = bquote(italic(.(first_two_words)) ~ .(rest_of_name) ~ "(ID " * .(species_ID) * ")"), theme = theme(plot.title = element_text(size = title_size, face = "bold", hjust = 0.5)))
  ...
  theme(plot.subtitle = element_text(color = "#636363"),
      ...
      plot_final<-plot1/plot2/plot3 + 
        plot_annotation(title = bquote(italic(.(first_two_words)) ~ .(rest_of_name) ~ "(ID " * .(species_ID) * ")"),
...
folders <- c("./plots/Report","./plots/FW Inverts","./plots/Marine","./plots/Plants", "./plots/TER Inverts" ,"./plots/Verts" )
grouped_pdfs <- c("Report.pdf", "fw_inverts_group.pdf","marine_group.pdf", "plants_group.pdf","ter_inverts_group.pdf", "verts_group.pdf"  )

# Use the full paths for output files
grouped_pdfs <- file.path("./plots/Report/grouped_pdfs", grouped_pdfs)

# Function to merge PDFs in chunks within a folder
merge_pdfs_in_chunks <- function(pdf_files, temp_output) {
  chunk_size <- 20 # Adjust based on system limits, try smaller if necessary
  merged_files <- c()
  ...
  for (i in seq(1, length(pdf_files), by = chunk_size)) {
    ...
    # Temporary file for the chunk merge
    temp_chunk_output <- tempfile(fileext = ".pdf")
    pdf_combine(input = chunk, output = temp_chunk_output)
    merged_files <- c(merged_files, temp_chunk_output)
  }
  
  # Merge all temporary chunk files into the final output
  pdf_combine(input = merged_files, output = temp_output)
...
for (i in seq_along(folders)) {
  # Get list of PDF files in the current folder
  pdf_files <- list.files(folders[i], pattern = "\\.pdf$", full.names = TRUE)
  
  # Merge PDFs in this folder into a single PDF for the group
  merge_pdfs_in_chunks(pdf_files, grouped_pdfs[i])
}


# Define the final output PDF file
final_pdf <- file.path("./plots/Report/HS_Report.pdf")

# Merge all grouped PDFs into the final document
pdf_combine(input = grouped_pdfs[1:2], output = "./plots/Report/HS_report_FW_Inverts.pdf")
pdf_combine(input = grouped_pdfs[c(1,3)], output = "./plots/Report/HS_report_Marine.pdf")
pdf_combine(input = grouped_pdfs[c(1,4)], output = "./plots/Report/HS_report_Plants.pdf")
pdf_combine(input = grouped_pdfs[c(1,5)], output = "./plots/Report/HS_report_TER_Inverts.pdf")
pdf_combine(input = grouped_pdfs[c(1,6)], output = "./plots/Report/HS_report_Verts.pdf")
```
